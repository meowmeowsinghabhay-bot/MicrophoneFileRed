...
export default function TeacherDashboard() {
  ...
  const loadData = async () => {
    ...
    if (!coursesRes.ok || !analyticsRes.ok) {
      const setupUrl = coursesData.setupUrl || analyticsData.setupUrl;
      setLoadError(
        setupUrl
          ? "Database not initialized. Run the one-time setup link from the login page."
          : coursesData.error || analyticsData.error || "Failed to load dashboard"
      );
      setLoading(false);
      return;
    }
  ...
  const createCourse = async () => {
    ...
    if (!res.ok) {
      ...
      setLoadError(data.error || "Failed to create course");
      return;
    }
    setShowCreate(false);
          ...
          <div className="grid gap-8 lg:grid-cols-2">
            <div>
              ...
              <div className="space-y-3">
                ...
                {courses.length === 0 && (
                  <p className="text-sm text-app-muted">No courses yet. Create one or run database setup.</p>
                )}
              </div>
            </div>
